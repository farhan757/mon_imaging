...
class NBController extends Controller
{
    ...
    public function index(Request $r)
    {
        ...
        $data = DB::table("imaging_master")
            ...
            ->where('imaging_master.product_id', 3)->groupBy('imaging_master_detail.no_account')->get();
        ...
        if ($r->download == "download") {
            ...
            $d = DB::table("imaging_master")
                ...
                ->where('imaging_master.product_id', 3)->groupBy('imaging_master.cycle')
            ...
            if(count($d) > 0){
                ...
                return view('imaging.NB.listdataimaging')->with('data', $data);
            }
            
        }

        \LogActivityUser::addToLog('View List NB');
        return view('imaging.NB.listdataimaging')->with('data', $data);
    }
    ...
    public function detail(Request $r)
    {
        ...
        $detail = DB::table('imaging_master_detail')->where('imaging_master_detail.id_master', '=', $id)->where('imaging_master_detail.no_account', '=', $nopol)
            ->get();
        \LogActivityUser::addToLog('Detail NB id : ' . $id . ' Nopol :' . $nopol);
        return view('imaging.NB.detail')->with(['detail' => $detail, 'value' => $master]);
    }
    ...
    public function delete(Request $r)
    {
        ...
        try {
            ...
            DB::commit();
            \LogActivityUser::addToLog('Sukses Delete NBController Nopol : ' . $nopol . ' Cyc :' . $cyc, 2);
            ...
            DB::rollBack();
            \LogActivityUser::addToLog('Gagal Delete NBController Nopol : ' . $nopol . ' Cyc :' . $cyc . ' Eror:' . $e, 2);
            return response()->json([
                'status' => 500,
                'message' => "Failed",
            ]);
